...
| 文件名 | 功能描述|
|--------|------|
|`2026hpn.py`|程序入口，负责初始化画布与主循环调度。|
|`torus.py`|3D 圆环渲染核心，包含矩阵变换与光照计算。|
|`firework.py`|烟花粒子系统，管理粒子的生命周期与物理运动。|
|`textrasterizer.py`|字符光栅化工具，将文本转为 ASCII 艺术。|
|`asciidrawer.py`|负责静态 ASCII 艺术画的渲染与透明叠加。|
|`houseascii.txt`|存储场景底部的建筑艺术画数据。|
